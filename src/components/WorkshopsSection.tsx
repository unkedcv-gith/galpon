      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 sm:space-y-8">
        ...
        <div className="bg-black/60 backdrop-blur-md rounded-3xl border-2 border-white/20 p-6 sm:p-10 text-center space-y-3 shadow-2xl text-white">
          ...
          <h2 className="font-heading text-3xl sm:text-4xl lg:text-5xl font-black text-white">
            Talleres <span className="text-[#F2C700] drop-shadow-[0_2px_10px_rgba(0,0,0,0.8)]">Deportivos</span> y Recreativos
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {WORKSHOP_PROGRAMS.filter(p => p.id !== 'por_un_dia').map((program) => {
            return (
              <div
                key={program.id}
              ...
              >
                ...
                {/* Enroll CTA */}
                <a
                  href={`${BRAND_INFO.whatsappUrl}?text=${encodeURIComponent(`Hola! Quisiera inscribir o consultar cupos para el taller "${program.title}" (${program.ageRange}).`)}`}
                  target="_blank"
